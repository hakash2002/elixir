defmodule Model do
  ...
  def save_predictions(predictions, file_path) do
    File.write!(
      file_path,
      "player_key,playername,ruled_out\n" <>
        Enum.map_join(predictions, "\n", fn %{
                                              player_key: player_key,
                                              player_name: player_name,
                                              prediction: prediction
                                            } ->
          "#{player_key},#{player_name},#{prediction}"
        end)
    )
  end
end

defmodule NaiveBayesClassifier do
  defstruct counts: %{}, total_count: 0

  def train(data) do
    data
    |> Enum.reduce(%NaiveBayesClassifier{}, fn entry, model ->
      days_category = Model.categorize_days(entry.days)
      update_model(model, days_category, entry.part)
    end)
  end

  def calculate_probabilities(%NaiveBayesClassifier{counts: counts, total_count: total_count}) do
  ...
  def get_total_count(%NaiveBayesClassifier{total_count: total_count}) do
    total_count
  end

  defp update_model(
         %NaiveBayesClassifier{counts: counts, total_count: total_count} = model,
         days_category,
         part
       ) do
    new_counts =
    ...
    %NaiveBayesClassifier{model | counts: new_counts, total_count: total_count + 1}
  end

  def calculate_ruled_out_probability(probabilities, players) do
    Enum.map(players, fn player ->
      days_category_prob =
        Map.get(probabilities.days_category_probabilities, player.days_category, 0)
  ...
  def average(model, days, part) do
    days_category = Model.categorize_days(days)
